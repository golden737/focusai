...
window.FlowApp = (() => {
  ...
  async function init() {
    try {
      ...
      _applyAccent(FlowState.get('user.id') ? FlowState.get('user.accentColor') : null);
  ...
  function _mountSidebar() {
    ...
    FlowState.subscribe('user.name', (name) => {
      _renderUserIdentity(name);
    });
    _renderUserIdentity(FlowState.get('user.name'));
    FlowState.subscribe('user.accentColor', () => _renderUserIdentity(FlowState.get('user.name')));
  ...
  function _renderUserIdentity(name) {
    const safeName = String(name || 'Usuario').trim() || 'Usuario';
    const el = document.getElementById('user-name');
    if (el) el.textContent = safeName;
    const av = document.getElementById('user-avatar');
    if (av) {
      av.textContent = safeName.substring(0, 2).toUpperCase();
      av.style.background = FlowState.get('user.accentColor') || 'var(--accent)';
    }
  }
  ...
  function _initAuth() {
    ...
    if (FlowSupabase.isConfigured()) {
      FlowSupabase.getUser().then(_hydrateAuthenticatedUser);
      FlowSupabase.onAuthChange(async (event, user) => {
  ...
  async function _hydrateAuthenticatedUser(user) {
    if (!user) return;
    let profile = null;
    try { profile = await FlowSupabase.getProfile(user.id); }
    catch (err) { console.warn('[Auth] No se pudo cargar el perfil:', err.message); }
    const name = profile?.name || user.user_metadata?.name || user.email?.split('@')[0] || 'Usuario';
    const accentColor = profile?.accent_color || '#6366f1';
    FlowState.update('user', { id: user.id, email: user.email, name, accentColor, role: profile?.role || 'user' });
    _applyAccent(accentColor);
    FlowState.persist();
  }
  ...
  async function logout() {
    ...
    FlowState.set('user.accentColor', null);
    _applyAccent(null);
    localStorage.clear();
    FlowRouter.navigate('login');
  }
