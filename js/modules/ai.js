...
SHOW_TOAST       → { message, type }`;
  ...
  function _friendlyHttpError(status, detail = '') {
    if (status === 404) return 'El túnel respondió, pero no encuentra la API de Ollama. Comprueba que Cloudflared apunte a http://localhost:11434.';
    if (status === 502 || status === 503) return 'Cloudflare no puede alcanzar Ollama. Verifica que Ollama y Cloudflared estén ejecutándose en tu PC.';
    if (status === 1033) return 'El túnel de Cloudflare ya no está disponible. Inicia Cloudflared de nuevo y pega la nueva URL.';
    if (status === 401 || status === 403) return 'Cloudflare bloqueó el acceso. Revisa las reglas de Cloudflare Access para permitir tu sitio.';
  ...
  function _friendlyConnectionError(error, baseUrl) {
    ...
    if (error?.message?.includes('Failed to fetch') || error?.message?.includes('NetworkError') || error instanceof TypeError) {
      return `No se pudo acceder a ${baseUrl}. Si el túnel abre pero falla en el navegador, configura CORS en Ollama para permitir el dominio de esta web.`;
    }
