...
window.FlowRouter = (() => {
  ...
  const DEFAULT_ROUTES = [
    { path: 'welcome',      view: 'views/welcome.html',      title: 'Organiza tu mejor día', icon: '✨' },
  ...
  function _parseHash(hash = '') {
    ...
    return { path: pathPart || 'welcome', query };
  }
  ...
  async function navigate(path, query = {}, options = {}) {
    ...
    if (!['welcome', 'login'].includes(path) && !FlowState.get('user.id')) {
      _isNavigating = false;
    ...
    const isPublicRoute = ['welcome', 'login'].includes(path);
    document.getElementById('app')?.classList.toggle('public-shell', isPublicRoute);
    // La marca usa su índigo en páginas públicas. Dentro de la cuenta,
    // cada persona recupera el acento elegido para su propia interfaz.
    FlowApp?.applyAccent(isPublicRoute ? null : FlowState.get('user.accentColor'));
  ...
  function init() {
    ...
    document.addEventListener('click', (e) => {
      ...
      const parsed = _parseHash(href);
      const target = view || parsed.path;
      if (target) navigate(target, parsed.query);
    });
