...
window.FlowRouter = (() => {
  ...
  // ── Carga de vistas ───────────────────────────────────────────────────────
    ...
    // 1. Buscar como <template> embebido en index.html
    const tpl = document.getElementById(`view-tpl-${route.path}`);
    if (tpl) return tpl.innerHTML;

    // 2. Buscar como <div> embebido
    const div = document.getElementById(`view-${route.path}`);
    if (div) return div.innerHTML;

    // 3. Fallback: fetch al archivo (local)
